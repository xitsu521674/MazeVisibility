...
void Maze::Draw_Wall(const float start[2], const float end[2], const float color[3])
{
	...
	float edge0[4] = { start[Y],0.0f,start[X],1 };
	float edge1[4] = { end[Y],0.0f,end[X],1 };

	float v1[4] = { edge0[X], 1.0f, edge0[Z], 1};
	float v2[4] = { edge1[X], 1.0f, edge1[Z], 1 };
	float v3[4] = { edge1[X], -1.0f, edge1[Z], 1 };
	float v4[4] = { edge0[X], -1.0f, edge0[Z], 1 };
	...
	if ((v1[3] > 0) || (v2[3] > 0) || (v3[3] > 0) || (v4[3] > 0)) {
		return;
	}
	for (int i = 0; i < 3; ++i) {
		v1[i] /= abs(v1[3]);
...
{
	if (f.leftEdge.end[X] == f.rightEdge.end[X] && f.leftEdge.end[Y] == f.rightEdge.end[Y])
		return;
	c->drawed = true;
	...
	for (int i = 0; i < 4; i++) {
		if (c->edges[i]->opaque) {
			...
			LineSeg plainE = f.clip(*(c->edges[i]),viewer_dir,viewer_fov);
			if (plainE.notDraw)
				continue;
			float edge_start[2] = {
					plainE.start[X],
			...
			float color[3] = { plainE.color[0],plainE.color[1] ,plainE.color[2] };
			Draw_Wall(edge_start, edge_end, color);
		}
		else {
			if (!(c->edges[i]->Neighbor(c)->drawed)) {
				/*if (c == view_cell) {
					while (false);
				}*/
				Cell* tmpc = c->edges[i]->Neighbor(c);
				for(int j=0;j<4;++j)
				if (c->edges[j]->endpoints[Edge::START]->posn[X] == 3 && c->edges[j]->endpoints[Edge::START]->posn[Y] == 10) {
					while (false);
				}
				LineSeg plainE = f.clip(*(c->edges[i]),viewer_dir,viewer_fov);
				...
				if (plainE.notDraw)
					continue;
				Frustum plainF = f.restricted(plainE);
				
				Draw_Cell(tmpc, plainF);
			}
		}
	}
	c->drawed = false;
...
void Maze::initFrustum(Frustum& f) {
	f.userX = viewer_posn[X];
	f.userY = viewer_posn[Y];
	LineSeg top(viewer_posn[X], viewer_posn[Y], viewer_posn[X] + cos(To_Radians(f.adjustDir(viewer_dir + viewer_fov / 2.0))), viewer_posn[Y] + sin(To_Radians(f.adjustDir(viewer_dir + viewer_fov / 2.0))));
	LineSeg bot(viewer_posn[X], viewer_posn[Y], viewer_posn[X] + cos(To_Radians(f.adjustDir(viewer_dir - viewer_fov / 2.0))), viewer_posn[Y] + sin(To_Radians(f.adjustDir(viewer_dir - viewer_fov / 2.0))));
	f.leftEdge = top;
	f.rightEdge = bot;
	//為了通過點在線上判定所以用edge的斜率來算交點 但是要取離viewer最近的點所以距離要跟viewer取



	/*LineSeg top(viewer_posn[X], viewer_posn[Y], viewer_posn[X]+cos(To_Radians(viewer_dir + viewer_fov / 2.0))*1000, viewer_posn[Y]+sin(To_Radians(viewer_dir + viewer_fov / 2.0))*1000);
	float deltaX;
	float deltaY;
	float tmp;
	float rec[2];
	...
	for (int i = 0; i < 4; ++i) {
		tmp = LineSeg(view_cell->edges[i]).Cross_Param(top);
		if (tmp <= 0.0)
			continue;
		deltaX = view_cell->edges[i]->endpoints[Edge::END]->posn[X] - view_cell->edges[i]->endpoints[Edge::START]->posn[X];
		deltaY = view_cell->edges[i]->endpoints[Edge::END]->posn[Y] - view_cell->edges[i]->endpoints[Edge::START]->posn[Y];
		float Ix = view_cell->edges[i]->endpoints[Edge::START]->posn[X] + deltaX * tmp;
		float Iy = view_cell->edges[i]->endpoints[Edge::START]->posn[Y] + deltaY * tmp;
		if ((Ix - viewer_posn[X]) * (Ix - viewer_posn[X]) + (Iy - viewer_posn[Y]) * (Iy - viewer_posn[Y]) < distance) {
			rec[X] = Ix;
			rec[Y] = Iy;
			std::cout << Ix << " " << Iy << "\n";
			distance = (Ix - viewer_posn[X]) * (Ix - viewer_posn[X]) + (Iy - viewer_posn[Y]) * (Iy - viewer_posn[Y]);
			indexRec = i;
		}
	}
	top.end[X] = rec[X];
	top.end[Y] = rec[Y];
	f.leftEdge = top;
	std::cout << "on edge test:" << (view_cell->edges[indexRec]->ON == view_cell->edges[indexRec]->Point_Side(rec[X], rec[Y])) << "\n";



	LineSeg bot(viewer_posn[X], viewer_posn[Y], viewer_posn[X] + cos(To_Radians(viewer_dir - viewer_fov / 2.0)) * 1000, viewer_posn[Y] + sin(To_Radians(viewer_dir - viewer_fov / 2.0)) * 1000);
	distance = 100000;
	for (int i = 0; i < 4; ++i) {
		tmp = LineSeg(view_cell->edges[i]).Cross_Param(bot);
		if (tmp <= 0.0)
			continue;
		deltaX = view_cell->edges[i]->endpoints[Edge::END]->posn[X] - view_cell->edges[i]->endpoints[Edge::START]->posn[X];
		deltaY = view_cell->edges[i]->endpoints[Edge::END]->posn[Y] - view_cell->edges[i]->endpoints[Edge::START]->posn[Y];
	...
	//std::cout << f.leftEdge.end[X] << " " << f.leftEdge.end[Y] << " " << f.rightEdge.end[X] << " " << f.rightEdge.end[Y] << "\n";
}
...
//======================================================================
void Maze::
Draw_View(const float focal_dist, const float fov)
//======================================================================
{
